...
extension UIStackView {
    public override func allSubviews() -> [UIView] {
        var list:[UIView] = []
        _ = self.subviews.map({ (view) in
            if view.isKind(of: UITextView.self) ||
                view.isKind(of: UILabel.self) ||
                view.isKind(of: UIImageView.self) ||
                view.isKind(of: UIButton.self) {
                list.append(view)
            } else {
                let ci = view.allSubviews()
                if ci.count > 0 {
                    list.append(contentsOf: ci)
                }
            }
        })
        return list
...
open class Loader
{
    fileprivate static func addLoaderToViews(_ views : [UIView])
    {
        CATransaction.begin()
    ...
    fileprivate static func removeLoaderFromViews(_ views: [UIView])
    {
        CATransaction.begin()
    ...
    open static func addLoaderListTo(_ view : UIView )
    {
        if let v = Bundle.main.loadNibNamed("CommonListFake", owner: nil, options: nil)?.first as? UIView{
            view.addSubview(v)
            v.boundInside(view)
            objc_setAssociatedObject(view, &vwXIB, v, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            let ci = v.ld_visibleContentViews()
            if ci.count > 0 {
                addLoaderToViews(ci)
            }
        }
    ...
    open static func addLoaderBlockTo(_ view : UIView )
    {
        if let v = Bundle.main.loadNibNamed("CommonFake", owner: nil, options: nil)?.first as? UIView{
            view.addSubview(v)
            v.boundInside(view)
            objc_setAssociatedObject(view, &vwXIB, v, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            let ci = v.ld_visibleContentViews()
    ...
    open static func addLoadMoreTo(_ view : UIView )
    {
        if let v = Bundle.main.loadNibNamed("LoadMoreFake", owner: nil, options: nil)?.first as? UIView{
            view.addSubview(v)
            v.boundInside(view)
            objc_setAssociatedObject(view, &vwXIB, v, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            let ci = v.ld_visibleContentViews()
    ...
    open static func removeLoaderFrom(_ view : UIView )
    {
        if let v = objc_getAssociatedObject(view, &vwXIB) as? UIView {
            removeLoaderFromViews(v.ld_visibleContentViews())
